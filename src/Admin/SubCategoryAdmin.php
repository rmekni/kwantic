...
final class SubCategoryAdmin extends AbstractAdmin
{
    protected function configureTabMenu(ItemInterface $menu, string $action, ?AdminInterface $childAdmin = null): void
    {
        ...
        $menu->addChild('View Sub-category', $admin->generateMenuUrl('show', ['id' => $id]));

        if ($this->isGranted('EDIT')) {
            $menu->addChild('Edit Sub-category', $admin->generateMenuUrl('edit', ['id' => $id]));
        }
    ...
    protected function configureFormFields(FormMapper $form): void
    {
        ...
        if ($subCategory && ($webPath = $subCategory->getImage())) {
            $fileFormOptions['help'] = '<img style="width: 150px;" src="' . $webPath . '" class="admin-preview"/>';
            $fileFormOptions['help_html'] = true;
        }
        $form
            ->add('category', ModelType::class, ['label' => 'Main category'])
            ->add('name')
